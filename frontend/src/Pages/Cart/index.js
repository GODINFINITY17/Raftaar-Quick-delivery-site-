...
const Cart = () => {
    ...
    const handleQuantityChange = (quantityVal, item) => {
        ...
        if (item._id && item._id.startsWith("mock-")) {
            context.setCartData((prev) => {
                const newCart = prev ? [...prev] : [];
                const existing = newCart.find(cartItem => cartItem._id === item._id);
                if (existing) {
                    existing.quantity = quantityVal;
                    existing.subTotal = existing.price * quantityVal;
                }
                return newCart;
            });
            setIsLoading(false);
    ...
    const removeItem = (id) => {
        ...
        if (id && id.startsWith("mock-")) {
            context.setCartData((prev) => prev.filter(item => item._id !== id));
            context.setAlertBox({
                open: true,
                error: false,
