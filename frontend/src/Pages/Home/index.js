...
import { getDisplayCategories } from "../../data/demoCategories";
...
const Home = () => {
  ...
  useEffect(() => {
    ...
    const fetchInitialData = async () => {
      try {
        ...
        setFeaturedProducts(
          mergeWithDemoProducts(Array.isArray(featuredRes) ? featuredRes : [])
        );
        setProductsData({
          ...productsRes,
          products: mergeWithDemoProducts(productsRes?.products || []),
        });
        setHomeSlides(
          Array.isArray(homeBannerRes) && homeBannerRes.length > 0
            ? homeBannerRes
            : DEMO_BANNERS
        );
        ...
        const cats = getDisplayCategories(context.categoryData);
        if (cats?.[0]?.name) {
          setSelectedCat(cats[0].name);
        }
      } catch (error) {
        console.error("Failed to fetch initial data:", error);
        setFeaturedProducts(DEMO_PRODUCTS.slice(0, 8));
        setProductsData({ products: DEMO_PRODUCTS });
        setHomeSlides(DEMO_BANNERS);
        setFilterData(DEMO_PRODUCTS.slice(0, 8));
      }
    };
      ...
      if (context.categoryData?.length > 0) {
        ...
        try {
          ...
          setRandomCatProducts({
            catName: context.categoryData[randomIndex]?.name,
            catId: context.categoryData[randomIndex]?.id,
            products: mergeWithDemoProducts(res?.products || []),
          });
        } catch (error) {
    ...
    const fetchFilteredProducts = async () => {
      if (selectedCat) {
        ...
        try {
          ...
          const merged = mergeWithDemoProducts(
  ...
  const displayCategories = getDisplayCategories(context.categoryData);
  ...
  return (
    <div className="home-page">
      ...
      <HomeCat catData={displayCategories} />
        ...
        <div className="container">
          <div className="row homeProductsRow">
            ...
            <div className="col-md-9 productRow">
              <div className="d-flex align-items-center res-flex-column">
                <div className="info" style={{ width: "35%" }}>
                  <h2 className="mb-0 hd">Order again</h2>
                  <p className="text-light text-sml mb-0">
                    Bestsellers near you — prices in ₹ (INR)
                  </p>
                </div>
              ...
              <div className="d-flex align-items-center mt-2">
                <div className="info w-75">
                  <h2 className="mb-0 hd">Fresh arrivals</h2>
                  <p className="text-light text-sml mb-0">
                    New stock added today — delivered in 10 minutes
                  </p>
                </div>
              </div>
          ...
          <div className="d-flex align-items-center mt-4">
            <div className="info">
              <h2 className="mb-0 hd">Top picks for you</h2>
              <p className="text-light text-sml mb-0">
                Handpicked deals — save more on every order
              </p>
            </div>
          </div>
        ...
        <div className="container">
          <div className="d-flex align-items-center mt-1 pr-3">
            <div className="info">
              ...
              <p className="text-light text-sml mb-0">
                Shop this category — free delivery above ₹199
              </p>
            </div>
