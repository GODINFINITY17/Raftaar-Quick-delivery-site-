...
const ProductModal = ({ data }) => {
    ...
    const handleWishlist = async () => {
        if (data?.id && data.id.startsWith("demo-")) {
            if (isInWishlist) {
                context.setMyListData(prev => prev.filter(item => item.productId !== data.id));
            } else {
                const wishlistItem = {
                    _id: "mock-" + Date.now(),
                    productTitle: data?.name,
                ...
                context.setMyListData(prev => [...prev, wishlistItem]);
            }
            ...
            <div className='row mt-2 productDetaileModal'>
                ...
                <div className='col-md-7'>
                    <div className='d-flex info align-items-center mb-3'>
                        {data?.oldPrice && (
                            <span className='oldPrice lg mr-2'>{formatINR(data?.oldPrice)}</span>
                        )}
                        <span className='netPrice text-danger lg'>{formatINR(data?.price)}</span>
                    </div>
